...
public class IntegralOdometry {
  ...
  private boolean moving(RelativeEncoder leftEncoder, RelativeEncoder rightEncoder) {
    if (Math.abs(Math.floor(leftEncoder.getVelocity() * 1000) / 1000) > 0
        || Math.abs(Math.floor(rightEncoder.getVelocity() * 1000) / 1000) > 0) {
      return true;
    } else
      return false;
  }

  private double getChassisVelocityX(double currentTime, double accelerationX, RelativeEncoder leftEncoder,
   ...
   */
  public double getChassisVelocityXWithTime(double accelerationX, RelativeEncoder leftEncoder,
      RelativeEncoder rightEncoder) {
  ...
  private Pose2d getPosition(double currentTime, Rotation2d currentAngle, double chassis_speedX) {
    ...
    var newPose = m_currentPosition.exp(
        new Twist2d(
